...
_URL_META_FORMAT = '{{scheme}}://{{hostname}}:{{port}}/0.1/directories/{entry_point}/default/{vendor}?{qs_prefix}xivo_user_uuid={{user_uuid}}{qs_suffix}'


def add_xivo_phonebook_url(raw_config, vendor, entry_point='input', qs_prefix='', qs_suffix=''):
...
def add_xivo_phonebook_url_from_format(raw_config, url_format):
    ...
    raw_config['XX_xivo_phonebook_url'] = url_format.format(scheme=scheme,
                                                             hostname=hostname,
                                                             profile='default',
                                                             port=port,
                                                             user_uuid=user_uuid)
