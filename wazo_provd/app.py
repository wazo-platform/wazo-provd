...
class ProvisioningApplication:
    ...
    def _dev_get_raw_config(self, device):
        ...
        if 'config' in device:
            ...
            return defer.ensureDeferred(self.device_raw_config_dao.get(cfg_id))
                        ...
                        if old_device_cfg and old_device_cfg.get('transient'):
                            ...
                            if not (
                                yield defer.ensureDeferred(
                                    self.device_dao.find_one(config=old_device_cfg_id)
        ...
        try:
            ...
            if 'config' in device:
                ...
                if device_cfg and device_cfg.get('transient'):
                    ...
                    if not (
                        yield defer.ensureDeferred(
                            self.device_dao.find_one(config=device_cfg_id)
                        )  # TODO DAO method
                    ):
                        yield defer.ensureDeferred(
