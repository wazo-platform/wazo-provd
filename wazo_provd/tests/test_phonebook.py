...
from wazo_provd.phonebook import (
    add_wazo_phonebook_url_from_format_v2,
    add_wazo_phonebook_url_v2,
...
class TestAddWazoPhonebookURLV2(unittest.TestCase):
    def setUp(self) -> None:
        self.raw_config: dict[str, Any] = {
            'X_xivo_phonebook_ip': '8.8.8.8',
            'X_xivo_user_uuid': '12-34',
        }

    def test_add_wazo_phonebook_url_v2_minimal(self) -> None:
        add_wazo_phonebook_url_v2(self.raw_config, 'acme')

        expected = 'http://8.8.8.8:9498/0.1/directories/input/default/acme/v2?xivo_user_uuid=12-34'
        assert_that(self.raw_config['XX_wazo_phonebook_url_v2'], equal_to(expected))

    def test_add_wazo_phonebook_url_v2_full(self) -> None:
        add_wazo_phonebook_url_v2(
            self.raw_config, 'acme', entry_point='ep', qs_prefix='p=1', qs_suffix='s=2'
        )
        ...
        expected = (
            'http://8.8.8.8:9498'
            '/0.1/directories/ep/default/acme/v2?p=1&amp;xivo_user_uuid=12-34&amp;s=2'
        )
        assert_that(self.raw_config['XX_wazo_phonebook_url_v2'], equal_to(expected))


class TestAddWazoPhonebookURLFromFormatV2(unittest.TestCase):
    def setUp(self) -> None:
        self.url_format = (
            '{scheme}://{hostname}:{port}/{profile}?xivo_user_uuid={user_uuid}'
        )
    ...
    def test_add_wazo_phonebook_url_from_format_v2_no_ip(self) -> None:
        del self.raw_config['X_xivo_phonebook_ip']

        add_wazo_phonebook_url_from_format_v2(self.raw_config, self.url_format)

        assert_that(self.raw_config, is_not(has_key('XX_wazo_phonebook_url_v2')))

    def test_add_wazo_phonebook_url_from_format_v2_no_user_uuid(self) -> None:
        del self.raw_config['X_xivo_user_uuid']
    ...
    def test_add_wazo_phonebook_url_from_format_v2_full(self) -> None:
        self.raw_config['X_xivo_phonebook_scheme'] = 'https'
        self.raw_config['X_xivo_phonebook_port'] = 4242
